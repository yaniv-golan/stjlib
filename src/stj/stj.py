...
class StandardTranscriptionJSON:
    ...
    @classmethod
    def from_file(cls, filename: str, validate: bool = False, raise_exception: bool = True) -> 'StandardTranscriptionJSON':
        """
        ...
        try:
            ...
            raise STJError(f"An unexpected error occurred while loading the file: {e}") from e
    ...
    def _deserialize_metadata(data: Dict[str, Any]) -> Metadata:
        ...
        created_at = datetime.fromisoformat(created_at_str.rstrip('Z')) if created_at_str else datetime.now(timezone.utc)
        ...
        if 'source' in data:
            ...
            source_additional_info = {k: v for k, v in source_data.items() if k not in {'uri', 'duration', 'languages'}}
            source = Source(
        ...
        metadata_languages = [Lang(code) for code in data.get('languages', [])] if data.get('languages') else None
        additional_info = {k: v for k, v in data.items() if k not in {'transcriber', 'created_at', 'source', 'languages', 'confidence_threshold'}}
        return Metadata(
            transcriber=transcriber,
    ...
    def _deserialize_transcript(data: Dict[str, Any]) -> Transcript:
        ...
        for s in data.get('styles', []):
            additional_info = {k: v for k, v in s.items() if k not in {'id', 'description', 'formatting', 'positioning'}}
            style = Style(
                id=s['id'],
        ...
        for s in data.get('segments', []):
            ...
            for w in s.get('words', []):
                word_additional_info = {k: v for k, v in w.items() if k not in {'start', 'end', 'text', 'confidence'}}
                word = Word(
                    start=w['start'],
                    end=w['end'],
                    text=w['text'],
            ...
            segment_additional_info = {k: v for k, v in s.items() if k not in {'start', 'end', 'text', 'speaker_id', 'confidence', 'language', 'style_id', 'words', 'word_timing_mode'}}
            segment = Segment(
                start=s['start'],
                end=s['end'],
                text=s['text'],
    ...
    def _custom_serialize(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            return {key: self._custom_serialize(value) for key, value in data.items() if value is not None}
    ...
    def _validate_speaker_and_style_ids(self) -> List[ValidationIssue]:
        ...
        valid_style_ids = {style.id for style in self.transcript.styles} if self.transcript.styles else set()
    ...
    def _validate_words_in_segment(self, segment: Segment, segment_idx: int) -> List[ValidationIssue]:
        """
        ...
        word_timing_mode = segment.word_timing_mode or (WordTimingMode.COMPLETE if words else WordTimingMode.NONE)
